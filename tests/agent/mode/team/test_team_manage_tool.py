"""Tests for team_configure tool — runtime member capability management.
...
- The lead cannot configure agents that have no source .md (in-memory)
"""
...
class TestTeamConfigureFrontmatterEdgeCases:
    ...
    async def test_yaml_null_list_returns_clean_error(self, tmp_path):
        """`mcp: ~` is rejected by AgentConfig; team_configure must fail safely.

        The schema treats `None` for a list field as invalid, not as an empty
        list. team_configure's `list` action re-parses via parse_agent_md and so
    ...
    async def test_list_reflects_pending_changes_before_reload(self, tmp_path):
        ...
        # And the in-memory agent state was NOT mutated by team_configure.
        assert team.members["executor"].agent.mcp_servers == []


# ---------------------------------------------------------------------------
# Integration: team_configure write → drift detection → in-place rebuild
# ---------------------------------------------------------------------------


class TestTeamConfigureDriftIntegration:
    ...
    async def test_add_tool_rebuilds_member_agent(self, tmp_path, monkeypatch):
        """team_configure add(tool) → refresh_if_dirty() → tool present on agent."""
...
class TestTeamConfigureToolMetadata:
    ...
    def test_each_call_returns_a_fresh_tool(self, basic_team):
        ...
        """
        a = make_team_configure_tool(basic_team)
        b = make_team_configure_tool(basic_team)
        assert a is not b
...
class TestTeamConfigureValidationPrecedence:
...
class TestTeamConfigureMcpPrefixFiltering:
...
class TestTeamConfigureFilesystemFailures:
    ...
    so every IO path through ``team_configure`` is wrapped to return a
...
class TestTeamConfigureListContract:
...
class TestTeamConfigureFrontmatterMoreShapes:
...
# ---------------------------------------------------------------------------
# Integration: loader soft-skip after team_configure grants then registry shifts
# ---------------------------------------------------------------------------


class TestTeamConfigureLoaderSoftSkip:
    ...
    soft-skip change. team_configure validates names *up front* against the
    ...
    async def test_unknown_tool_after_grant_is_warned_and_skipped(
        ...
        team, member, md = TestTeamConfigureDriftIntegration._build_real_member(
