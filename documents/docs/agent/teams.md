...
| Workflow | receive → plan with todos; mark dependencies in task content (`blocked by task_1`); delegate peer handoffs from the dependency graph; wait (or `<sleep>`) → synthesise | receive → work → `team_message` peers for help if needed → `team_message` results to lead or next peer → `<sleep>` |
