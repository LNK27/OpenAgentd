...
| Workflow | receive → plan with todos using `dependencies` and `assigned_to`; spawn/message only unblocked owners; delegate peer handoffs from the dependency graph; wait (or `<sleep>`) → synthesise | receive → claim assigned todo if provided → work when unblocked → `team_message` peers for help if needed → `team_message` results to lead or next peer → `<sleep>` |
