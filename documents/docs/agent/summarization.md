...
| **Cache-first LLM call** | The summariser keeps the normal request prefix, including the actual system prompt after prompt hooks and tool definitions, and uses the session id as the prompt-cache key so large compactions can reuse provider prompt cache. Only the summary instruction is appended as the final user message. The prompt explicitly asks for summary text only and not tool use. |
