...
OpenAgentd memory should be a simple, editable, LLM-maintained markdown wiki compiled from canonical raw sources, with raw fallback search and benchmarkable retrieval quality.
...
`{OPENAGENTD_MEMORY_DIR}` initially reuses the existing wiki root setting/path unless renamed later.

```text
{OPENAGENTD_MEMORY_DIR}/
  SCHEMA.md
  INDEX.md
  LOG.md
...
Add/update manual commands around memory and Dream:
...
uv run python -m manual.memory_bench longmemeval --mode raw --limit 20 --data PATH
uv run python -m manual.memory_bench longmemeval --mode wiki --limit 20 --data PATH
uv run python -m manual.memory_bench longmemeval --mode wiki-plus-raw --limit 20 --data PATH
```
...
10. Update docs and `manual/AGENTS.md`.
