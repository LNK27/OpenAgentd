...
2. Run targeted tests and `manual.memory_bench` on local datasets.
...
- Synthetic OpenAgentd preference/context set for fast regression.
...
Negative/unanswerable rows are supported in `manual.memory_bench` with `negative: true`, `abstain: true`, `answerable: false`, `should_answer: false`, or an empty answer list. The harness remains dataset-local: no downloads, pass a local file through `--data PATH`.
