...
OpenAgentd still needs product-level semantic-memory primitives before choosing a backend:
...
```bash
uv run python -m manual.memory vector rebuild
uv run python -m manual.memory vector search "query"
uv run python -m manual.memory_bench longmemeval --mode semantic --data PATH
uv run python -m manual.memory_bench longmemeval --mode hybrid --data PATH
```
