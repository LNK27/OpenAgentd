...
import { SplitGrid } from "@/components/TeamChatView/SplitGrid"
...
function renderGrid(agentNames: string[], streams = makeStreams(agentNames)) {
  const result = render(
    <SplitGrid
...
describe("SplitGrid automatic layout", () => {
  it("renders one agent as a single full-height column", () => {
    const { root } = renderGrid(["lead"])

    expect(columnTexts(root)).toHaveLength(1)
    expect(screen.getAllByText("lead")).toHaveLength(2)
  })

  it("adds a second spawned agent as a side-by-side column", () => {
    const { root } = renderGrid(["lead", "executor#1"])
  ...
  it("places the third spawned agent under the right column", () => {
    const { root } = renderGrid(["lead", "executor#1", "reviewer#1"])
  ...
  it("grows to three columns at five agents and stacks extra panes to the right", () => {
  ...
  it("ignores transient roster entries that do not have streams yet", () => {
    const { root } = renderGrid(["lead", "executor#1"], makeStreams(["lead"]))
